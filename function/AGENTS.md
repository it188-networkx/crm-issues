...
## 【强制规则】每个 Feature 必须设计 `04_业务流程` 用例

> **背景**：多次出现「只写了页面冒烟/交互/控件用例，忽略业务流程用例」的遗漏（如 POC License 只写了列表+筛选、待办只写了展示、跟进只写了详情区块）。此后每次设计用例必须强制覆盖业务流程。

1. **每个 feature 至少 1 个 `04_业务流程` 用例**，覆盖该 feature 的核心端到端闭环。仅当 feature 确无业务流程（纯展示/纯配置）时，须在 README「Out of Scope」写明原因。
2. **写业务流程用例前，必须先在浏览器打开原型**（`crm-product/prototype/full-prototype-standalone.html`），点击走完该 feature 的完整操作链路（创建→提交→审批→状态流转→终态），再据此设计用例步骤。
3. **业务流程用例的步骤编号必须连贯**（S1→S2→...），覆盖「入口→操作→中间态→终态」，而非孤立的元素断言。
4. **状态机类功能**（商机阶段、POC 审批流、任务状态、周报确认流、客户状态）必须画出 `状态流转图`（mermaid）并逐条覆盖每个状态转换。

### 常见业务流程清单（设计用例时逐项自查）

| Feature | 必覆盖的业务流程 |
| :--- | :--- |
| 商机 | 创建→阶段推进→成交/失单→退回/重新激活；POC License 创建→提交→审批→驳回重提→标记开通 |
| 跟进记录 | 新建→填内容→归档校验→AI规范化→归档锁定→转商机建议→生成待办 |
| 客户/联系人 | 建档→联系人新增→决策链设置→状态联动客情降级→客户状态流转 |
| 待办任务 | 创建→状态流转（待办→进行中→已完成/已取消）→到期提醒 |
| 周报 | 生成→销售确认→审核修改→标记已阅→锁定→回写关联档案 |
